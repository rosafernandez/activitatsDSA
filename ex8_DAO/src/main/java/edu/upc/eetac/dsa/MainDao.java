...
public class MainDao
{
    public static void main( String[] args ) throws NoSuchMethodException, IllegalAccessException, InvocationTargetException, SQLException {
        ...
        //user.select2("rosa","lloret"); //el select es fa pel (name i address) no pel numero de user
        //user1.delete(); //per borrar poses el user q vols borrar
        System.out.println("\n**********************************************************");
        //List<User> l = DAO.getAllUsers(); //mostrar llista d tots els users
        List<User> l = DAO.getAllUsersWithAddress(); //mostrar llista d tots els users q tinguin adreça
